<template>
  <div id="recipe">
    ...
    <ul>
      <li v-for="ingredient in ingredients" :key="ingredient.name" class="text-left pl-16">
        <p>- {{ ingredient.name }} <b>{{ ingredient.amount }}</b></p>
      </li>
    </ul>
  </div>
</template>
...
export default class RecipePreview extends Vue {
  ...
  mounted() {
    axios.get<Recipe>(`/recipe/${this.$route.params.id}`).then(res => {
      console.log(res)
